...
describe('Player tests', () => {
    ...
    test("should reject player types that are not `real` or `computer`", () => {
        ...
        expect(() => { new Player('abcdef') }).toThrow(Error);
        expect(() => { new Player('human') }).toThrow(Error);
    });

    test('should reject player types that are not strings', () => {
        expect(() => { new Player(undefined)}).toThrow(Error);
        expect(() => { new Player(null)}).toThrow(Error);
